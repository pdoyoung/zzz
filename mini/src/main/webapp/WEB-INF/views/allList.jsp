...
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
	<%@page session="true" %> <!-- 서버의 세션 영역에 변수에 접근 가능하도록 -->
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
		...
		<tbody>
			<c:forEach items="${tListitems }" var="foodvo">
				...
				<tr>
					...
					<td rowspan="6" class="border"><a href="comment?no=${foodvo.no }">댓글달기</a></td>
				</tr>
				<tr>
					<td class="border">${foodvo.no }</td>
					<td class="border"> ${foodvo.name } </td>
					<td class="border">${foodvo.zone }</td>
					<td class="border"><a href="viewDetail?no=${foodvo.no }">${foodvo.username }</a></td>
				</tr>
				<tr>
				...
				<c:forEach items="${commentListitems }" var="commentvo">			
				<tr>
				<td class="border">${commentvo.cname }</td>
				<td class="border">${commentvo.text }</td>
				</tr>
